...
init python:
    config.keymap['game_menu'] = []  # Desactiva ESC y clic derecho
    # Elimina las teclas asignadas a quick save/load
...
image bg jungle dense2 = im.Scale("bg jungle dense2.jpg", config.screen_width, config.screen_height)
...
image bg huerta_exterior sunset = im.Scale("bg huerta_exterior sunset.jpg", config.screen_width, config.screen_height)
image bg huerta_exterior = im.Scale("bg huerta_exterior.jpg", config.screen_width, config.screen_height)
image bg construccion_barrera = im.Scale("bg construccion_barrera.jpg", config.screen_width, config.screen_height)
image bg bg jungle river = im.Scale("bg bg jungle river.jpg", config.screen_width, config.screen_height)
image bg barranco = im.Scale("bg barranco.jpg", config.screen_width, config.screen_height)
image bg jungle tall_trees = im.Scale("bg jungle tall_trees.jpg", config.screen_width, config.screen_height)
image bg manglar_interior = im.Scale("bg manglar_interior.jpg", config.screen_width, config.screen_height)
image bg manglar_rodeo = im.Scale("bg manglar_rodeo.jpg", config.screen_width, config.screen_height)
image bg manglar_serpiente = im.Scale("bg manglar_serpiente.jpg", config.screen_width, config.screen_height)
image bg playa_gruta_entrada = im.Scale("bg playa_gruta_entrada.jpg", config.screen_width, config.screen_height)
image bg gruta_entrada = im.Scale("bg gruta_entrada.jpg", config.screen_width, config.screen_height)
image bg cueva_raices = im.Scale("bg cueva_raices.jpg", config.screen_width, config.screen_height)
image bg cave_sand = im.Scale("bg cave_sand.jpg", config.screen_width, config.screen_height)
image bg gruta_pasillo = im.Scale("bg gruta_pasillo.jpg", config.screen_width, config.screen_height)
image bg gruta_agua = im.Scale("bg gruta_agua.jpg", config.screen_width, config.screen_height)
image bg cueva_angosta = im.Scale("bg cueva_angosta.jpg", config.screen_width, config.screen_height)
image bg gruta_marca_agua = im.Scale("bg gruta_marca_agua.jpg", config.screen_width, config.screen_height)
image bg cueva_humedad_profunda = im.Scale("bg cueva_humedad_profunda.jpg", config.screen_width, config.screen_height)
image bg gruta_excavacion_inicio = im.Scale("bg gruta_excavacion_inicio.jpg", config.screen_width, config.screen_height)
image bg pasillo_agua_altura_pecho = im.Scale("bg pasillo_agua_altura_pecho.jpg", config.screen_width, config.screen_height)
image bg gruta_inundada = im.Scale("bg gruta_inundada.jpg", config.screen_width, config.screen_height)
...
default bebio = False
default comio = False
default reporte_despierta_antes = False
default reporte_todos_despiertos = False
default marina_ofrece_comida = False
default reporte_bob_se_queda = False
default reporte_marina_se_queda = False
default reporte_laura_se_queda = False
default reporte_encontraron_agua = False
...
default reporte_va_con_bob = False
default reporte_va_con_marina = False
default reporte_va_con_laura = False
default reporte_exploran_tres = False
default reporte_exploran_todos = False
...
default reporte_destino_exploracion_1 = ""
default reporte_destino_exploracion_2 = ""
...
default reporte_apoyo_laura = False
default reporte_apoyo_ingrid = False
default reporte_apoyo_charles = False
default reporte_estrategia_agresiva = False
default reporte_estrategia_pacifica = 0
default reporte_estrategia_pasiva = 0
default player = 0
default reporte_apoyo_tomas = 0
default reporte_apoyo_bob = 0
default reporte_apoyo_erika = 0
default reporte_apoyo_marina = 0
default grupo_jugador = []
default grupo_jabaporco = []
default grupo_bob = []
default grupo_erika = []
default reporte_apoyo_lider_jugador = 0
default reporte_apoyo_lider_bob = 0
default reporte_apoyo_lider_erika = 0
...
default reporte_enfoque_preparacion = ""
default reporte_equipo_bob = False
default reporte_equipo_erika = False
default reporte_equipo_jugador_opcion1 = False
default reporte_equipo_jugador_opcion2 = False
default reporte_jugador_es_lider = False
default reporte_bob_es_lider = False
default reporte_erika_es_lider = False
default elegido = ""
default reporte_decision_cueva = ""
...
default reporte_compartio_info_caja = None  # Puede ser True o False
...
default reporte_equipo_exploracion = []
default reporte_decision_rocas_tardia = ""
default reporte_reloj_marea = 0                # Tiempo narrativo antes del cierre de la cueva
default reporte_cap12_choice = ""
...
default reporte_postura_jugador_desaparecidos = ""
default reporte_ruta_elegida = ""
default decision_tras_manglar = ""
default reporte_cap11_hallazgo = ""
default reporte_cap11_discusion = ""
default reporte_cap11_organizar = ""
default reporte_cap11_rescate_charles = ""
default reporte_cap11_regreso_laura = ""
default reporte_cap11_charles_joyas = ""
default reporte_cap11_rapido_laura = ""
default reporte_cap11_media_erika = ""
default reporte_cap11_tardia_tiburon = ""
...
default relaciones_cap5_bob = 99
default relaciones_cap5_marina = 99
default relaciones_cap5_laura = 99
default relaciones_cap5_erika = 99
...
label pedir_codigo_capitulo:
    while True:  # Se repite hasta que el ID sea válido
        ...
        if resultado:
            ...
            if renpy.android:  # Solo mostrar en Android   
                if persistent.cantidad_capitulos == 4 and resultado == "44ug":
                    jump chapter_5_start
                elif persistent.cantidad_capitulos == 8 and resultado == "88yn":
...
label nutrir_ingrid:
    if (agua > 0 and not bebio) or (comida > 0 and not comio):
        if reporte_todos_despiertos == False:
        ...
        menu:
            ...
            "Tratar de calmarla y esperar a que despierten los demás." if reporte_despierta_antes and not reporte_todos_despiertos:
                y "Tranquila, Ingrid, estás bien, te golpeaste la cabeza. El capitán Bob, Marina y Laura estamos aquí."
                jump despiertan_todos
        
            "Con eso se va a sentir un poco mejor." if not reporte_despierta_antes and (comio or bebio):
                y "Eso te hará bien."
        ...
        if reporte_todos_despiertos:
            jump ingrid_debil
        else:
            if reporte_despierta_antes:
                jump despiertan_todos
            if not reporte_despierta_antes:
                jump dejarla_descansar
...
label p5_explorar:
    if reporte_todos_explorar:
        ...
        if reporte_encontrar_agua_comida:
            ...
            if not stuff_bidon_agua and reporte_bob_se_queda:
                show laura hablando at right
                with Dissolve(0.5)
            ...
            elif not stuff_bidon_agua and (reporte_laura_se_queda or reporte_marina_se_queda):
                show bob parado hablando at center
                with Dissolve(0.5)
...
label p5_division_tareas:
    if reporte_encontrar_agua_comida and reporte_va_con_laura:
    ...
    elif reporte_encontrar_agua_comida and reporte_va_con_marina:
    ...
    elif reporte_encontraron_agua and reporte_va_con_laura:
    ...
    elif reporte_encontraron_agua and reporte_va_con_marina:
...
label arroyo_frutos:
    show bg jungle river at truecenter
    with Dissolve(.5)
    if reporte_bob_se_queda and not reporte_encontrar_agua_comida:
        show marina sonriendo at left
        with Dissolve(.5)
    ...
    elif reporte_marina_se_queda and not reporte_encontrar_agua_comida:
    ...
    elif reporte_laura_se_queda and not reporte_encontrar_agua_comida:
        show marina sonriendo at left
        with Dissolve(.5)
        m "¡Sabía que lo lograríamos si perseverábamos!"
        show bob parado hablando at right
    ...
    elif reporte_bob_se_queda and reporte_encontrar_agua_comida:
        show marina sonriendo at left
        with Dissolve(.5)
        m "¡Agua, al fin!"
        show laura sonriendo at right
    ...
    elif reporte_marina_se_queda and reporte_encontrar_agua_comida:
        show laura sonriendo at right
        with Dissolve(.5)
        l "¡Agua, al fin!"
        show bob pensando at left
    ...
    elif reporte_laura_se_queda and reporte_encontrar_agua_comida:
        show marina sonriendo at right
        with Dissolve(.5)
        l "¡Agua, al fin!"
        show bob pensando at left
    ...
    elif reporte_va_con_marina and not reporte_encontrar_agua_comida:
        show marina sonriendo at left
        with Dissolve(.5)
        m "¡Sabía que lo lograríamos si perseverábamos!"
    ...
    elif reporte_va_con_laura and not reporte_encontrar_agua_comida:
        show laura sonriendo at left
        with Dissolve(.5)
        l "El agua está fresca y clara, ¡y parece haber suficientes frutos para que todos podamos comer hoy!"
        $ choice_position = "superior" # default alta superior
    ...
    elif reporte_va_con_marina and reporte_encontrar_agua_comida:
        show marina sonriendo at left
        with Dissolve(.5)
        m "¡Agua, al fin!"
    ...
    elif reporte_va_con_laura and reporte_encontrar_agua_comida:
        show laura sonriendo at left
        with Dissolve(.5)
        l "¡Agua, al fin!"
        y "Está fresca y clara."
...
label manantial_marina_laura:
    ...
    elif reporte_exploran_tres and reporte_bob_se_queda:
        show laura sonriendo at right
        with Dissolve(.5)
        l "¡Aquí está! El manantial del que les hablamos."
        show marina sonriendo at left
    ...
    elif reporte_exploran_tres and reporte_marina_se_queda:
        show laura sonriendo at right
        with Dissolve(.5)
        l "¡Aquí está! El manantial del que les hablamos."
        show bob pensando at left
    ...
    elif reporte_exploran_tres and reporte_laura_se_queda:
        show marina sonriendo at left
        with Dissolve(.5)
        m "¡Aquí está! El manantial del que les hablamos."
...
label retorno_refugio:
    ...
    if comida <=2 and (reporte_exploran_todos or reporte_exploran_tres):
    ...
    elif reporte_exploran_todos or reporte_exploran_tres:
    ...
    if reporte_exploran_todos:
    ...
    elif reporte_exploran_tres:
        if reporte_laura_se_queda or reporte_bob_se_queda:
            show marina sonriendo at left
            with Dissolve(.5)
            m "¡Vamos! Ingrid necesita comer y beber algo."
            hide marina
        ...
        if reporte_va_con_bob:
            ...
            if reporte_encontrar_agua_comida or reporte_encontraron_agua:
...
label cap7_sed:
    ...
    if len(opciones_exploracion) == 1:
        $ reporte_destino_exploracion_1 = opciones_exploracion[0]
        ...
        menu:
            ...
            "[opciones_texto[1]]":
                $ reporte_destino_exploracion_1 = opciones_exploracion[1]
                jump cap7_decidir_quien_va
...
label explorar_primer_sitio:
    if reporte_destino_exploracion_1 == "cueva":
    ...
    elif reporte_destino_exploracion_1 == "cabaña":
    ...
    elif reporte_destino_exploracion_1 == "claro":
...
label cap7_antes_de_encuentro_nuevos:
    ...
    if climb_hill: # o sea, si se descubrió el claro en el capítulo 3
        ...
        python:
            ...
            if refugio != "cabaña" and reporte_destino_exploracion_1 != "cabaña":
                opciones_exploracion2.append("cabaña")

            if refugio != "cueva" and reporte_destino_exploracion_1 != "cueva":
                opciones_exploracion2.append("cueva")

            if refugio != "claro" and reporte_destino_exploracion_1 != "claro":
                opciones_exploracion2.append("claro")

        $ reporte_destino_exploracion_2 = opciones_exploracion2[0]
        ...
        $ reporte_destino_exploracion_2 = "claro"
    else:
        y "Cuando estabamos buscando refugio, nadie decidió explorar hacia el oeste."
        y "Quizá en esa dirección haya algo interesante."
        $ reporte_destino_exploracion_2 = "claro"

    if reporte_destino_exploracion_2 == "cueva":
        scene bg jungle cave
    elif reporte_destino_exploracion_2 == "cabaña":
        scene bg jungle hut
    elif reporte_destino_exploracion_2 == "claro":
        scene bg jungle clearing
...
label cap7_evaluacion_refugio:
    ...
    if reporte_destino_exploracion_2 == "cabaña":
...
label cap8_avisar_tormenta:
    ...
    menu:
        "Apoyar el enfoque de Erika: organizar el refuerzo estructural del refugio.":
            $ reporte_enfoque_preparacion = "estructura"
            $ erika += 2
        ...
        "Apoyar el enfoque de Bob: asegurarse de tener provisiones antes que nada.":
            $ reporte_enfoque_preparacion = "recursos"
            $ bob += 2
        ...
        "Equilibrar ambos enfoques. Se debe trabajar en todo a la vez.":
            $ reporte_enfoque_preparacion = "equilibrado"
            $ bob += 1
            $ erika += 1
...
label cap8_prioridades_refugio:
    ...
    if reporte_enfoque_preparacion == "estructura":
    ...
    elif reporte_enfoque_preparacion == "recursos":
    ...
    elif reporte_enfoque_preparacion == "equilibrado":
...
label cap8_separacion_grupo:
    ...
    if reporte_apoyo_bob > reporte_apoyo_erika:
    ...
    elif reporte_apoyo_erika > reporte_apoyo_bob:
...
label cap8_formacion_equipos:
    ...
    menu:
        ...
        "Proponer una alternativa, tomando parte de la idea de Erika pero integrando el entorno.":
            $ reporte_equipo_jugador_opcion2 = True
...
label cap8_finalizacion_proteccion:
    ...
    if reporte_equipo_jugador_opcion1:
    ...
    if reporte_equipo_jugador_opcion2:
...
label cap8_eleccion_liderazgo:
    ...
    menu:
        ...
        "Apoyar a Erika como líder":
            ...
            $ reporte_apoyo_lider_erika += 3
            "{i}Erika cruza los brazos con firmeza.{/i}"
            jump cap8_dialogo_lider_erika

        "Tomar el liderazgo":            
            $ reporte_apoyo_lider_jugador += 3
            $ reporte_jugador_es_lider = True
            ...
            menu:
                ...
                "Intentar que Erika se una a tu grupo":
                    if reporte_apoyo_lider_erika >= 0:
                        y "Aquellos que piensen lo mismo, vengan conmigo."
...
label cap8_formacion_grupos_finales:
    ...
    python:
        ...
        for personaje in ["tomas", "marina", "charles", "laura"]:
            ...
            elif reporte_apoyo_lider_bob > reporte_apoyo_lider_erika:
    ...
    menu:
        "Intentar convencer a Ingrid de quedarse":
            ...
            if getattr(store, "reporte_apoyo_ingrid", 0) > 2:
...
label cap8_end:
        ...
        hide screen relaciones_popup with dissolve
        # hide screen decisiones_popup with dissolve
        "Pronto estará disponible el último segmento donde las desiciones serán clave no solo para sobrevivir sino para resolver el misterio de la isla."
        jump continuar
...
label cap9_mision_equipo_3:
    ...
    if miembro_extra == "marina":
        if marina > 1:
            show marina sonriendo at center
            with Dissolve(0.5)
            m "Creo que elegiste una tarea acorde a tus habilidades. Eres muy ingenios[e]."
        elif marina < -1:
...
label cap9_reaccion_pasiva:
    ...
    if "erika" in grupo_jugador or reporte_erika_es_lider:
    ...
    elif "bob" in grupo_jugador or reporte_bob_es_lider:
...
label cap9_encuentro_caja:
    ...
    menu:
        ...
        "Ignorarlo, estás agotado y quieres irte ya.":
            ...
            if "bob" in grupo_jugador or reporte_bob_es_lider:
                show bob pensando at right
                with Dissolve (0.5)
            ...
            elif "erika" in grupo_jugador or reporte_erika_es_lider:
                show erika conversando at right
                with Dissolve (0.5)
...
label cap10_inicio_discusion_cueva:
    ...
    menu:
        "No necesitan más misterios. Necesitan estabilidad. Y que todos conserve sus energías.":
            $ reporte_decision_cueva = "ignorar"
        ...
        "Lo que viste te intriga. Pero sería un error arrastrar al grupo sin preparación. Primero, lo urgente.":
            $ reporte_decision_cueva = "restaurar_primero"
...
label cap10_asignacion_tareas:
    ...
    if reporte_jugador_es_lider:
        ...
        menu:
            ...
            "Revisar los recipientes de agua que se dejaron en el borde del arroyo.":
                $ tarea_jugador1 = "agua"
                jump cap10_tarea_agua_1

    else:  # Jugador no es líder
        if reporte_erika_es_lider:
...
label cap10_tarea_troncos_1:
    ...
    if "charles" not in grupo_jugador:
        ...
        $ cruce1 = "ingrid"
        show ingrid relajada at right with Dissolve(0.5)
        ...
        $ cruce1 = "laura"
        show laura tranquila at right with Dissolve(0.5)
    ...
    menu:
        "Contarle la verdad sobre lo que había en la caja.":
            $ reporte_compartio_info_caja = True
            if cruce1 == "charles":
        ...
        "Ocultar y minimizar lo hallado.":
            $ reporte_compartio_info_caja = False
            if cruce1 == "charles":
...
label cap10_tarea_agua_1:
    ...
    scene bg jungle river with Dissolve(0.5)
    show screen combined_ui

    "{i}La caminata hacia el arroyo no es larga.{/i}"
    "{i}Los recipientes improvisados de agua siguen en su sitio, llenos de hojas húmedas y barro filtrado.{/i}"
    "{i}Mientras buscas algo para limpiarlos, sientes que no estás sol[e].{/i}"
...
label cap10_tarea_troncos_3:
    ...
    "{i}Percibes que no vino aquí para discutir... sino para medirte. Quiere saber en qué punto estás parad[e] con respecto al resto.{/i}"
...
label cap10_tarea_lena_3:
    ...
    "{i}Llegas al lugar donde se cruzaron por primera vez con Tomas, recogiendo leña. Fue hace muy poco pero se siente como muy lejano.{/i}"
    "{i}Mientras removés un madero para limpiarlos, sientes que no estás sol[e].{/i}"
...
label cap10_preparacion_reunion_general:
    ...
    y "¿Alguien movió el cuaderno? Lo dejé en la caja envuelto con la manta. Y ahora no está."
    ...
    "{i}Un rato después, todos se reúnen.{/i}"
...
label cap10_reunion_doble_grupo:    
    ...
    m "Había algo importante y se mantuvo en secreto."
    m "Eso probablemente sea lo más cuestionable."
    m "Fue una mala idea. Y ahora Laura y Charles han tenido otra mala idea."
    hide marina with Dissolve(0.5)
    ...
    menu:
        ...
        "Intentas negociar para que se retome la calma y se piense en conjunto.":
            ...
            y "Tal vez no sea justo pedírselos luego de lo que pasó, pero debemos trabajar juntos para encontrarlos."
    ...
    "{i}El clima de la conversación es nefasto, pero lo importante ya está dicho y no pueden quedarse esperando.{/i}"
    y "Los dos deben haber salido en dirección al mar. Tal vez hacia esa playa con los acantilados."
...
label cap10_formacion_equipo_exploracion:
    ...
    if not reporte_equipo_exploracion:
        $ reporte_equipo_exploracion = []
    ...
    menu:
        ...
        "No parece que nadie quiera ir contigo" if (invitar_marina and invitar_bob and invitar_erika and invitar_ingrid and invitar_tomas and len(reporte_equipo_exploracion) == 0):
            jump cap10_soledad_post_fallo
        "Solo una persona aceptó sumarse" if (invitar_marina and invitar_bob and invitar_erika and invitar_ingrid and invitar_tomas and len(reporte_equipo_exploracion) == 1):
            jump cap10_soledad_post_fallo
...
label cap10_soledad_post_fallo:
    ...
    if len(reporte_equipo_exploracion) == 1:

        "{i}Sientes una mano en el hombro que trata de confortarte.{/i}"

        if "bob" in reporte_equipo_exploracion:
            ...
            $ reporte_equipo_exploracion.append("marina")

        elif "erika" in reporte_equipo_exploracion:
            ...
            $ reporte_equipo_exploracion.append("tomas")
            ...
            $ reporte_equipo_exploracion.append("ingrid")
...
label cap10_invitar_marina:
    ...
    if ("marina" in grupo_jugador and marina >= 0) or ("marina" not in grupo_jugador and marina > 0):
        show marina sonriendo at center with Dissolve(0.4)
    ...
    if len(reporte_equipo_exploracion) == 2:
        jump cap10_exploracion_inicio
    else:
        jump cap10_formacion_equipo_exploracion
...
label cap10_invitar_bob:
    ...
    if ("bob" in grupo_jugador and bob >= 0) or ("bob" not in grupo_jugador and bob > 0):
        ...
        $ reporte_equipo_exploracion.append("bob")
    else:
        show bob parado serio at center with Dissolve(0.4)
...
label cap10_invitar_erika:
    ...
    if ("erika" in grupo_jugador and erika >= 0) or ("erika" not in grupo_jugador and erika > 0):
        ...
        $ reporte_equipo_exploracion.append("erika")
        hide erika with Dissolve(0.4)
    else:
...
label cap10_exploracion_inicio:
    ...
    if "ingrid" in reporte_equipo_exploracion:
        show ingrid seria at left with Dissolve(0.4)
        ...
        show bob parado hablando at left with Dissolve(0.4)
        ...
        show marina alerta at left with Dissolve(0.4)
    ...
    if "marina" in reporte_equipo_exploracion:
        show marina alerta at centerleft with Dissolve(0.4)
        m "Cuidemos nuestras energías, ¿si? No sabemos qué nos espera cuando los encontremos."
        hide marina with Dissolve(0.4)

    elif "erika" in reporte_equipo_exploracion:
        show erika parada at centerleft with Dissolve(0.4)
        k "¿Y si fue uno de ellos quien convenció al otro?"
        hide erika with Dissolve(0.4)

    elif "tomas"in reporte_equipo_exploracion:
        show tomas cruzado at centerleft with Dissolve(0.4)
        t "No me gusta esto. Si se fueron sin decir nada, ¿cómo podemos confiar en ellos?"
        hide tomas with Dissolve(0.4)

    elif "bob" in reporte_equipo_exploracion:
        show bob parado hablando at centerleft with Dissolve(0.4)
        b "Espero que los encontremos antes de que hagan alguna locura."       
        hide bob with Dissolve(0.4)

    elif "ingrid" in reporte_equipo_exploracion:
        show ingrid seria at centerleft with Dissolve(0.4)
    ...
    menu:
        "Sentenciar que probablemente se fueron por puro egoísmo o falta de criterio. No se los puede justificar.":
            $ reporte_postura_jugador_desaparecidos = "critico"
        ...
        "Sugerir que tal vez tenían razones válidas. No es justo juzgar sin saber.":
            $ reporte_postura_jugador_desaparecidos = "comprensivo"
        ...
        "Admitir que tu también hubieras sentido la tentación de ser el primero en ir a descubrir el misterio.":
            $ reporte_postura_jugador_desaparecidos = "autocritico"
    ...
    if "tomas" in reporte_equipo_exploracion:
    ...
    if "marina" in reporte_equipo_exploracion:
        show marina hablando at center with Dissolve(0.4)
...
label cap10_exploracion_jungla:
    ...
    "{i}Luego de avanzar un buen rato ven un zurco en la vegetación, señal clara de que alguien pasó hace poco por ahí.{/i}"
    ...
    if "tomas" in reporte_equipo_exploracion:
        ...
        show erika enojada at left with Dissolve(0.4)
    ...
    scene bg jungle tall_trees with Dissolve(0.5)
...
label cap10_espera_en_rocas:
    ...
    if "erika" in reporte_equipo_exploracion:
        show erika gr enojada at leftgr with Dissolve(0.4)
...
label cap10_manglar_decisiones:
    ...
    menu:
        ...
        "Rodear la laguna y no correr riesgos.":
            ...
            hide screen combined_ui
            show screen combined_ui
            scene bg manglar_rodeo with Dissolve(0.5)
    ...
    scene bg manglar_serpiente with Dissolve(0.5)
    "{i}Entre dos troncos, enroscada como una rama más, una serpiente se camufla perfectamente. Respiras profundamente.{/i}"
    ...
    menu:
        ...
        "Avanzar ignorándola, con cuidado de no alertarla.":
    ...
    scene bg playa_gruta_entrada with Dissolve(0.5)
...
label cap10_inicio_cueva:
    ...
    if "bob" not in reporte_equipo_exploracion:
        show bob parado hablando at left with Dissolve(0.4)
...
label chapter_11_start:
    ...
    show ingrid gr cintura at leftgr with Dissolve(0.4)
    i "Lo único que nos falta es que alguien se tropiece entre las rocas y se tuerza el tobillo."

    show marina hablando at centerright with Dissolve(0.4)
    ...
    scene bg cueva_raices with Dissolve(0.5)
    show erika enojada at left with Dissolve(0.4)
    ...
    if reporte_jugador_es_lider:
        ...
        if tomas > 0:
            ...
            show tomas serio at right with Dissolve(0.4)
    ...
    hide marina with Dissolve(0.4)
    hide tomas with Dissolve(0.4)
    scene bg cave_sand with Dissolve(0.5)
    ...
    show ingrid triste  at center with Dissolve(0.4)   
    ...
    menu:
        "Minimizar el hallazgo, evitando sugerir escenarios pesimistas.":
            $ reporte_cap11_hallazgo = "Minimizar el hallazgo"
        ...
        "Mencionar que seguramente se detuvieron para atender las heridas de Laura.":
            $ reporte_cap11_hallazgo = "Atender las heridas de Laura"
        ...
        "Admitir que el hallazgo te preocupa.":
            $ reporte_cap11_hallazgo = "Admitir que te preocupa"
...
label cap11_dialogos_en_marcha:
    ...
    m "Laura tuvo ideas raras desde que llegamos, pero arriesgarse así, con la marea subiendo, es lo más irracional que ha hecho."
    ...
    show bob parado enojado at centerright with Dissolve(.4)
    ...
    show bob parado serio at centerright with Dissolve(.4)
    show erika conversando at centerleft with Dissolve(.4)
    k "Quizás hay algo más, pero no creo que creyeran que iban a encontrar en esta cueva como salir de la isla."
    show marina triste at right with Dissolve(.4)
    ...
    menu:
        "Defender a Laura.":
            ...
            $ reporte_cap11_discusion = "Defender a Laura"
            $ laura += 1
            $ marina += 1
        ...
        "Criticar a Charles.":
            $ reporte_cap11_discusion = "Criticar a Charles"
        ...
        "Enfocarse en el rescate.":
            $ reporte_cap11_discusion = "Enfocarse en el rescate"
    ...
    scene bg gruta_agua with Dissolve(0.5)
    "{i}Pronto caminan con el agua cubriéndoles los pies.{/i}"
    ...
    show ingrid gr seria at rightgr with Dissolve(.4)
    ...
    show marina preocupada at center with Dissolve(.4)
    m "¿A qué te refieres?"
...
label cap11_marca_de_la_marea:
    ...
    scene bg gruta_marca_agua with Dissolve(0.5)
    show screen combined_ui
    ...
    "{i}El sonido de una ola golpeando la roca no muy lejos los sorprende a todos. La marea está subiendo.{/i}"
    "{i}La situación se vuelve más complicada, segundo a segundo.{/i}"    

    menu:
        "Intentar calmarlos a todos.":
            y "Tranquilos, los encontraremos pronto."
            $ reporte_cap11_organizar = "Intentar calmarlos a todos"
            $ marina += 1
            $ bob -= 1
        ...
        "Pedirle a todos que se mantengan alerta.":
            $ reporte_cap11_organizar = "Pedirle a todos que se mantengan alerta"
        ...
        "Plantear regresar por donde vinieron.":
            $ reporte_cap11_organizar = "Plantear regresar por donde vinieron"
            ...
            "{i}Tomas se encoje de hombros y el resto te evita con su mirada.{/i}"
            "{i}Tal vez sea demasiado tarde como para proponer regresar.{/i}"
            #Acá hay que agregar una opcion de volverse solo, ahi tiene que haber una opcion de volver a entrar o simplemente espera afuera a quye todos salgan.   
    hide erika with Dissolve(0.4)
    hide marina with Dissolve(0.4)
    hide bob with Dissolve(0.4)
...
label cap11_entrada_cueva:
    ...
    scene bg cueva_humedad_profunda with Dissolve(0.5)
    show screen combined_ui

    "{i}Luego de un par de vueltas más, el pasaje se abre a una amplia caverna.{/i}"
    "{i}Acurrucada detrás de una roca caída, con el pie vendado, está Laura, que sonríe aliviada al verlos{/i}"

    show laura sonriendo at left with Dissolve(0.4)
    ...
    i "Se fue solo, ¿eh? Ni un poco de sentido común."
    i "Hay señales por todos lados de que toda la cueva queda inundada."
    ...
    menu:
        "Ir solo tras Charles.":
            $ reporte_cap11_rescate_charles = "Ir a rescatarlo solo"
            y "Yo voy a traer a Charles."
            y "Bob, Tomás, ustedes dos carguen a Laura. Igrid, Marina, Erika, ustedes vayan adelante para ver que sea seguro."
            $ erika += 1
            $ bob += 1
            $ ingrid += 1
            $ marina += 1
        ...
        "Quedarte a ayudar a Laura.":
            ...
            $ reporte_cap11_rescate_charles = "Cuidar a Laura"
            show bob parado serio at center with Dissolve(.4)
            ...
            "{i}Erika, Ingrid y Marina van un poco adelante buscando el camino mas seguro.{/i}"
            jump cap11_regreso_con_laura

label cap11_regreso_con_laura:

    scene bg gruta_agua with Dissolve(.4)
    show screen combined_ui
    ...
    show laura gr hablando at rightgr with Dissolve(0.4)
    ...
    menu:
        "Cargar a Laura en brazos":
            $ reporte_cap11_regreso_laura = "Cargar a Laura en brazos"
            $ laura += 1
            "{i}Levantas a Laura con cuidado y ella cruza sus brazos alrededor de tu cuello.{/i}"
            "{i}Avanzas pero lentamente, los demás no parecen estar muy de acuerdo.{/i}"

        "Hacer un esfuerzo entre todos para que sus pies no toquen el agua ni el piso.":
            $ reporte_cap11_regreso_laura = "Cargar a Laura entre todos"
            $ laura += 1
            "{i}Se reparten alrededor de Laura, se aseguran de agarrarla firmemente y la levantan en el aire.{/i}"
            "{i}Coordinan el ritmo de la marcha como si lo hubieran ensayado.{/i}"

        "Proponer separarse.":
            $ reporte_cap11_regreso_laura = "Proponer separarse"
    ...
    scene bg cueva_lago at truecenter with Dissolve(0.5)
    "{i}Un rato despues se escuchan pasos chapoteando tras ustedes. Voltean y ven a Tomas, alcanzándolos." 

    show tomas serio at centerleft with Dissolve(0.4)
    t "¡Pensé que ya estarían fuera! Charles viene atrás. Lo encontré justo a tiempo."
  

    show charles triste at left with Dissolve(0.4)
...
label cap11_busqueda_charles:
    ...
    show charles dedos v at right with Dissolve(0.4)
    c "{i}Sabía que estaban aquí. Mira el símbolo tallado... es el mismo del dibujo. Las joyas fueron escondidas aquí mismo.{/i}"

    "{i}En el fondo del pozo hecho por Charles se ve una piedra chata de gran tamaño con un simbolo tallado.{/i}"
    show charles boca abierta at right with Dissolve(0.4)
    c "Llegas justo a tiempo, ayúdame a mover esa piedra."
    "{i}Charles y tú retiran con gran esfuerzo la roca revelando un atado de cuero casi destruído.{/i}"
    ...
    menu:
        "Ayudar a Charles a escarbar. No pueden irse con tan poco, luego de arriesgar tanto.":
            $ reporte_cap11_charles_joyas = "Ayudar a Charles a escarbar"
            $ charles += 1
            $ reporte_reloj_marea +=1
            "{i}Juntos remueven barro y arena hasta que se topan con un cofre de madera podrida, que se hace trozos con solo tocarlo.{/i}"
        ...
        "Hay que irse. Lo encontrado debe bastar o la marea no los perdonará.":
            $ reporte_cap11_charles_joyas = "Irse ya con lo que se encontró"
            $ charles -= 1
...
label cap11_salida_rapida_abertura:
    ...
    menu:
        "La sujetas con fuerza, ayudándola a trepar.":
            $ reporte_cap11_rapido_laura = "Ayudas a Laura a trepar"
            $ laura += 1
        ...
        "Le haces señas a Charles para que la asista, mientras tú abres camino, tanteando la firmeza de las rocas y apartando raíces.":
            $ reporte_cap11_rapido_laura = "Que Charles la asista"
        ...
        "Vigilas desde arriba mientras los demás la ayudan. La coordinación es clave.":
            $ reporte_cap11_rapido_laura = "Coordinas desde arriba"
            $ bob -= 1
...
label cap11_salida_media_abertura:
    ...
    show erika gr conversando at shoulder with Dissolve(0.4)
    ...
    menu:
        "Te quedas dudando, una mala desicion puede causar mas problemas":
            $ reporte_cap11_media_erika = "Duda"
            $ tomas += 1
            $ erika += 1
        ...
        "Das instrucciones rápidas, organizando al resto para que aflojen las rocas del fondo, o tiren de Erika para liberarla.":
            $ reporte_cap11_media_erika = "Da instrucciones"
            $ erika += 1
            $ ingrid += 1
        ...
        "Esperas a que Bob tome la iniciativa.":            
            $ reporte_cap11_media_erika = "Espera que Bob actúe"
            "{i}Como buen marino, Bob se zambuye y afloja las piedras que sujetan el pie de Erika y ella se libera.{/i}"
            
    show erika gr sonriendo at waterleft with Dissolve(0.4)
    k "¿Libre! Justo a tiempo, gracias. ¡Sigamos!"
    hide erika with Dissolve(0.4)
    ...
    scene bg gruta_inundada at truecenter with Dissolve(0.5)
    ...
    menu:
        ...
        "Esperar a que el agua suba un poco más y los eleve hacia el manglar.":
            ...
            scene bg gruta_inundada2 at truecenter with Dissolve(0.5)
            y "Con el aumento de la marea, aprovecharemos el impulso para subir un poco más."
...
label cap11_salida_media_salida:
    ...
    if charles <= 0:
        ...
        c "Se que se arriesgaron mucho por nosotros, sin ustedes hubieramos estado en problemas."
    ...
    menu:
        ...
        "Acompañar a Tomas en el reproche. Fue una pésima idea que vinieran solos.":
            $ tomas += 1  
            $ charles -= 1
            $ laura -= 1          
...
label cap11_salida_tardia_tension:
    ...
    menu:
        "Coordinas el grupo en cadena para que cada uno que empiece a trepar pueda ayudar al siguiente.": 
            $ reporte_cap11_tardia_tiburon = "Coordina una cadena de ayuda"
        ...
        "Empujas a Charles primero, luego trepas tú para ayudar a Laura.":   
            $ reporte_cap11_tardia_tiburon = "Sube Charles y ayudas a Laura primero"         
        ...
        "Dejas que Bob suba primero con Laura, y luego cubres al resto mientras trepa.": 
            $ reporte_cap11_tardia_tiburon = "Sube último"            
...
label cap11_post_salida_media:
    ...
    scene bg manglar_inicial with Dissolve(0.5)
    show screen combined_ui
    $ update_stat("sed", sed - 1)
    $ show_variable_changed_popup("La sed ha aumentado ", rojo)
    ...
    if charles < 0:
        show charles gr enojado at rightgr with Dissolve(0.4)
        c "Lo que hice no fue por codicia. Era una oportunidad. Si ustedes decidieron venir detrás nuestro, háganse cargo."
    else:
        show charles gr boca abierta at rightgr with Dissolve(0.4)
        c "No era la idea ponerlos en peligro. Quisimos investigar y fuimos."
    
    show charles gr brazos cruzados at rightgr with Dissolve(0.4)
    c "Por otro lado, conseguimos recuperar una buena cantidad de joyas. Asi que no salió tan mal."
...
label cap12_discusion_rescate:
    ...
    show laura gr enojada at leftgr with Dissolve(0.4)
    l "¡Era un avión de búsqueda! ¡Lo vi claramente! Dio una curva y se fue."
    show laura seria at left with Dissolve(0.4)
    show charles triste at centerleft with Dissolve(0.4)
    c "Sí, pero no bajó ni hizo señales. Sin duda ni nos vió."
   

    show erika conversando at centerright with Dissolve(0.4)
    ...
    show erika enojada at centerright with Dissolve(0.4)
    k "Lo que pasó con la caja fue grave. Todos sabíamos que había algo importante ahí."
    ...
    show laura enojada at left with Dissolve(0.4)
    l "No teníamos que pedir permiso. Vimos una oportunidad y la tomamos."

    show charles dedos v at centerleft with Dissolve(0.4)
    show laura seria at left with Dissolve(0.4)
    ...
    menu:
        ...
        "Intentar mediar entre ambos lados":
            $ reporte_cap12_choice = "Intentar mediar entre ambos lados"
            $ bob += 1
            $ tomas += 1
        ...
        "Quedarse en silencio":
            $ reporte_cap12_choice = "Quedarse en silencio"
            "{i}Preferís observar. Algunos lo valoran y otros no.{/i}"
            $ tomas += 1
            $ ingrid += 1
            $ laura += 1
            $ bob -= 1
...
label cap12_reacciones_jugador:
    ...
    if reporte_cap12_choice == "Defender a Laura y Charles":
        show laura sonriendo at left with Dissolve(0.5)
        hide ingrid with Dissolve(0.5)
    ...
    m "¿Cómo que hacemos, las entregamos a la policía. Es lo más lógico."
...
##### este no va, quedo por las dudas
    ...
    "Bob, Laura y Marina se inclinan por construir la balsa. Erika, Ingrid y Tomas creen que mantener la hoguera encendida es más seguro. Charles aún no ha decidido."

    menu:
        "Unirme al equipo que construye una balsa con Bob, Laura y Marina":
            $ reporte_proyecto = "balsa"
...
label cap12_hoguera_evaluacion:
    ...
    y "No se puede, si no estamos todos juntos no hay forma de terminar este proyecto."
    y "Necesitamos decidir como grupo qué hacer."
    "Llaman al otro equipo para deliberar entre todos."
    jump cap12_decision_grupal_dialogos
...
label cap12_decision_grupal_reasignacion:
    ...
    if marina > 1:
        ...
        if jugador_proyecto == "balsa":
            show marina preocupada at right with Dissolve(0.5)
            m "No onfío mucho en vos pero me sumo a la balsa"
            $ equipo_jugador += 1
        else:
            show marina triste at right with Dissolve(0.5)
    ...
    "La mayoria se compromete con el proyecto de la balsa."
    jump cap12_balsa_completa
...
label cap_final_balsa_rescate:
    ...
    hide tomas with Dissolve(0.5)
    show bob gr pensando at rightgr with Dissolve(0.5)
    b "No parece haber mucho aquí..."

    show erika gr parada at leftgr with Dissolve(0.5)
...
label epilogo_isla:
    ...
    "Has completado con éxito la aventura, tú y tus compañeros naufragos han sobrevivido y regresan a sus hogares."
    jump cap12_reporte
